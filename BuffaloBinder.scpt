-- Script modified by Daniel James 6/29/16, 1:47 PM
...
display dialog "If the Mac has a sticker to be registered with the Boulder Police Department, enter the number here:" default answer pd
set pd to text returned of result

display dialog "PD sticker no. is: " & pd
...
set hardware to model & " - s/n#" & cereal & space & "- PD#" & pd & " - " & cpu & speed & space & mem & " RAM" & " - " & macMACWiFi & space & macMACeth1 & space & macMACeth2 -- & space & macMACp2p & space & macMACfw0
...
try -- this is the beginning of the entire binding process.  The 'end try' is at the end of this script.  Everything else below is in between.
	...
	try
		if blocko = "" then
			...
			display dialog "You are not currently bound to an Active Directory." buttons {"You don't say?"}
		else
			set reBound to true
		end if
	end try
	...
	repeat while reBind is true
		...
		display dialog "Now, we are attempting to bind to Active Directory the Mac with the name " & hostessKid with icon note buttons {"Carry On!", "Rather Not"} default button "Carry On!" cancel button "Rather Not"
		if result = {button returned:"Carry On!"} then
			
			-- display dialog "Enter the fully qualified name of Active Directory" default answer "ad.colorado.edu"
			
			set doughMain to "ad.colorado.edu"
			...
			try
				...
				do shell script "dsconfigad -f -a " & hostessKid & space & "-domain " & doughMain & " = -u " & joiner & " -p " & noSecret & " -ou \"CN=Computers,DC=ad,DC=COLORADO,DC=edu\"" with administrator privileges
...
-- Group C should be whatever institutional admin group should be added by default
set groupC to quote & "OIT - DS - DDS East Campus Pod - R - Technician Super Users,OIT - DS - Computer Admins Casper" & quote
